...
function Blackjack() {
    ...
    function AIValueBuilder(id, currentDeck) {
      ...
      return new Promise(async (resolve) => {
        ...
        if (difficulty === "easy") {
          ...
          let potentialSuccess = await ValueRemainingCards(
            currentDeck,
            goalDiff
          );
          while (potentialSuccess > 0.4) {
            let newCard = await PopNewCard(currentDeck, newHand);
            newHand = [...newHand, newCard];
            value = await AcquireValue(newHand);
            goalDiff = 21 - value;
  ...
  function AcquireWinner() {
    ...
    return new Promise((resolve) => {
      ...
      for (let i = 0; i < playerDecks.length; i++) {
        if (
          playerDecks[i].value > yourHandValue &&
          playerDecks[i].value <= 21
        ) {
          resolve("You Lose!");
        }
      }
          ...
          >
            <div>
              <Card.Body>
                ...
                <Button
                  variant={difficulty === "medium" ? "success" : "secondary"}
                  onClick={() => setDifficulty("medium")}
                >
                  Medium
                </Button>{" "}
                <Button
                  variant={difficulty === "hard" ? "success" : "secondary"}
            ...
            <div>
              <Card.Body>
                ...
                <Button
                  variant={aiCount === 1 ? "success" : "secondary"}
                  onClick={() => setAICount(1)}
                >
                  1
                </Button>{" "}
                <Button
                  variant={aiCount === 2 ? "success" : "secondary"}
                  onClick={() => setAICount(2)}
                >
                  2
                </Button>{" "}
                <Button
                  variant={aiCount === 3 ? "success" : "secondary"}
      ...
      return (
        <div>
          <Card className="GameOverScreen">
            ...
            <Card.Body>
              Congratulations! You may not have beat the game, but you denied it
              to have any power over you. Let this success mark another step in
              you taking back control of your own life!
            </Card.Body>
          </Card>
        </div>
      );
      ...
      return (
        <div>
          <Card className="GameOverScreen">
            ...
            <Card.Body>
              You did it, you have somehow turned 500 chips into 100,000. But
              take time now to look back at what it cost, how much time was
              spent during the process? How much was lost for you to reach this
              point? Finally, was it worth it? Thankfully, aside from the time,
              very little was lost, but what you have gone through in this small
              game is a simple display of an addiction that millions of people
              go through every day. Hopefully, this experience has encouraged
              you to stray away from this addiction. Thank you for playing.
            </Card.Body>
          </Card>
        </div>
      );
    ...
    return (
      <div>
        <Card className="GameOverScreen">
          ...
          <Card.Body>
            You have run out of chips, despite the numerous steps taken to
            continue playing. Consider this however, despite time, you have lost
            nothing. The same cannot be said for many people who suffer from
            gambling addictions every year. Hopefully, this experience has
            encouraged you to stray away from this addiction. Thank you for
            playing.
          </Card.Body>
        </Card>
      </div>
          ...
          <Card className="ImageOverlayStyle">
            ...
            <Card.ImgOverlay>
              ...
              <Card.Text className="PlayerBtnSetPos">
                ...
                <Button
                  variant="secondary"
                  disabled={winMessage !== "" || yourHand.length === 0}
                  onClick={SetWinner}
                >
                  Stay
                </Button>{" "}
